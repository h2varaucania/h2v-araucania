...
- **2026-07-04** — **NOINDEX pre-lanzamiento** (Carlos: circular el link para feedback sin que Google lo indexe aún). Interruptor por env var `SITE_INDEXABLE`: ausente/≠true ⇒ `noindex,nofollow` por meta (layout.tsx) y por header `X-Robots-Tag` en todas las respuestas (next.config.ts). El `robots.txt` deja el crawl permitido a propósito (Google debe poder leer la página para respetar el noindex). No afecta el link directo. **AL LANZAR:** `SITE_INDEXABLE=true` en Vercel + redeploy. typecheck OK; verificado en producción.
