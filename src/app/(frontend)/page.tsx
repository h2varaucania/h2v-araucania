...
export default async function Home() {
  let hero = { titulo: 'Hidrógeno Verde en La Araucanía', subtitulo: 'Plataforma informativa sobre los avances, proyectos y oportunidades del hidrógeno verde en la región de La Araucanía, Chile.', ctaPrimario: 'Conozca el Programa', ctaSecundario: 'Ver Proyectos en el Mapa' };
  ...
  try {
    ...
    if (data?.seccionExplora?.cards?.length > 0) {
      cards = data.seccionExplora.cards.map((c: any, i: number) => ({
        titulo: c.titulo,
  ...
  return (
    <>
      <section className="relative py-20 md:py-32 px-4" style={{ background: "linear-gradient(135deg, #0D7377 0%, #1B3A5C 100%)" }}>
        <div className="max-w-4xl mx-auto text-center text-white">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/programa/quienes-somos" className="px-8 py-3 bg-white text-[#1B3A5C] font-semibold rounded-full hover:bg-white/90 transition-colors">{hero.ctaPrimario}</Link>
      ...
      <section className="py-16 px-4 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl md:text-3xl font-bold text-[#1B3A5C] text-center mb-12">{seccionTitulo}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {cards.map((card) => (
              <Link key={card.enlace} href={card.enlace} className="group bg-white rounded-xl p-6 shadow-sm hover:shadow-lg border border-gray-100 hover:border-[#0D7377]/30 transition-all duration-200">
                ...
                <h3 className="text-lg font-semibold text-[#1B3A5C] mb-2 group-hover:text-[#0D7377] transition-colors">{card.titulo}</h3>
